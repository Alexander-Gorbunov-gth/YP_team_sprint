...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER INDEX idx_session_user_id RENAME TO idx_session_old_user_id;"
    )
    ...
    op.create_index(
        "idx_sessions_jti", "sessions", ["jti", "device_type"], unique=True
    )
    op.create_index(
        "idx_sessions_refresh_token",
        "sessions",
        ["refresh_token", "device_type"],
        unique=True,
    )
    op.create_index("idx_sessions_user_id", "sessions", ["user_id"])

    op.execute(
        "CREATE TABLE IF NOT EXISTS sessions_desktop PARTITION OF sessions FOR VALUES IN ('desktop');"
    )
    op.execute(
        "CREATE TABLE IF NOT EXISTS sessions_smart PARTITION OF sessions FOR VALUES IN ('smart');"
    )
    op.execute(
...
def downgrade() -> None:
    ...
    op.create_index(
        "idx_session_user_id", "sessions", ["user_id"], unique=False
    )
