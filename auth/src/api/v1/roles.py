...
async def get_all_roles(permisson_service: PermissionService = Depends(get_permission_service)):
    return permisson_service.get()


@roles_rouner.get("/{slug}/", response_model=[PermissionResponse])
async def get_role(slug: str, permisson_service: PermissionService = Depends(get_permission_service)):
    return permisson_service.get(slug=slug)


@roles_rouner.post("/", response_model=PermissionResponse)
async def create_role(data: PermissionCreate, permisson_service: PermissionService = Depends(get_permission_service)):
...
async def delete_role(slug: str, permisson_service: PermissionService = Depends(get_permission_service)):
