...
class PermissionService:
    ...
    async def create_or_update(
        self, data: PermissionBase, slug: str | None = None
    ) -> Permission:
        """Создаёт новое разрешение или обновляет существующее"""
        existing_permission = await self._permission_repository.get_permission(
            slug
        )
        ...
        if existing_permission:
            ...
            return await self._permission_repository.update_permission(
                existing_permission
            )
        else:
            logger.info(f"Создание нового разрешения: {data.slug}")
            return await self._permission_repository.create_permission(
