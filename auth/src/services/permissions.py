...
from src.schemas.permissions import PermissionCreate, PermissionUpdate
...
class PermissionsService:
    ...
    async def create(
        self, data: PermissionCreate, db: AsyncSession = Depends(get_session)
    ):
        obj_in_db = await db.scalar(
            exists().where(Permission.slug == data.slug).select()
        )
        if obj_in_db:
            raise HTTPException(
                status_code=409, detail=f"Объект с slug - {data.slug} уже существует"
            )
    ...
    async def list(self, db: AsyncSession = Depends(get_session)):
        permissions = await db.scalars(select(Permission))
