...
from src.api.v1.schemas.address import (
    AddressCreateSchema,
...
async def get_my_address(
    address_service: FromDishka[IAddressService], user: CurrentUserDep
):
...
async def delete_address(
    address_service: FromDishka[IAddressService], user: CurrentUserDep, address_id: UUID
) -> bool:
...
@router.patch(
    "/{address_id}", summary="Изменить адрес", response_model=AddressResponseSchema
)
async def update_address(
    ...
    address = await address_service.update_address(
        address_dto, user_id=user.id, address_id=address_id
    )
    return AddressResponseSchema(**address.model_dump())
