...
#     response_model=list[SubscriptionRepresentSchema],
# )
...
# @router.post(
#     "/", summary="Подписаться на автора", response_model=SubscriptionRepresentSchema
# )
# async def subscribe(
#     subscription_service: FromDishka[ISubscriptionService],
#     user_id: UUID | str,
...
async def unsubscribe(
    ...
    await subscription_service.delete_subscription(
        SubscriptionDeleteSchema(user_id=user, host_id=data.host_id)
    )
    return True
