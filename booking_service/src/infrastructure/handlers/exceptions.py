...
exception_handlers: dict[type[Exception], Callable[[Request, Any], Coroutine[Any, Any, Response]]] = {
    ...
    EventNotOwnerError: create_exception_handler(
        status_code=status.HTTP_403_FORBIDDEN, detail="Недостаточно прав для выполнения операции."
    ),
    EventStartDatetimeError: create_exception_handler(
        status_code=status.HTTP_400_BAD_REQUEST, detail="Некорректная дата начала события."
    ),
