...
class IAddressService(ABC):
    ...
    @abstractmethod
    async def get_address_by_id(self, address_id: UUID) -> Address: ...

    @abstractmethod
    async def get_my_addresses(self, user_id: UUID) -> Iterable[Address]: ...

    @abstractmethod
    async def update_address(self, address: AddressUpdateDTO) -> Address: ...

    @abstractmethod
    async def delete_address(self, address_id: UUID) -> Address: ...
...
class AddressService(IAddressService):
    ...
    async def update_address(self, address: AddressUpdateDTO) -> Address:
        ...
        async with self._uow as uow:
            updated_address = await uow.address_repository.update(address=address)
            if updated_address is None:
                logger.warning("Адрес с id=%s не найден", address.id)
                raise AddressNotFoundError("Адрес не найден")
