...
class IBaseFeedbackService(abc.ABC):
    ...
    async def set_review(
        self,
        feedback: EventFeedbackCreateDTO | EventFeedbackUpdateDTO | UserFeedbackCreateDTO | UserFeedbackUpdateDTO
    ) -> EventFeedback:
        ...

    @abc.abstractmethod
    async def delete(self, feedback: EventFeedbackDeleteDTO | UserFeedbackDeleteDTO) -> None: ...
...
class EventFeedbackService(IEventFeedbackService):
    ...
    async def set_review(
        ...
        async with self._uow as uow:
            ...
            if updated_feedback is None:
                logger.info(f"Отзыв с {feedback.event_id=}, {feedback.user_id=} не найден. Создаем новый.")
    ...
    async def get(self, id: UUID | str) -> list[EventFeedback]:
        async with self._uow as uow:
            return await uow.event_feedback_repository.get_id(id=id)
...
class UserFeedbackService(IUserFeedbackService):
    ...
    async def set_review(
        ...
        async with self._uow as uow:
            if feedback.owner_id == feedback.user_id:
                raise UserFeedbackSelfError(
                    "Невозможно оставить отзыв на самого себя."
                )

            check = False
            current_events = await uow.event_repository.get_events_by_user_id(feedback.owner_id)
            for current_event in current_events:
                ...
                for reservation in current_event.reservations:
                    if reservation.user_id == feedback.user_id and reservation.status == ReservationStatus.SUCCESS:
                        check = True
                        break
            if not check:
            ...
            if updated_feedback is None:
                logger.info(f"Отзыв с {feedback.owner_id=}, {feedback.user_id=} не найден. Создаем новый.")
    ...
    async def get(self, id: UUID | str) -> list[UserFeedback]:
        async with self._uow as uow:
            return await uow.user_feedback_repository.get_id(
                id=id
            )
