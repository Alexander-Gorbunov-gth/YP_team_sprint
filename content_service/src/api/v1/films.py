...
def create_response_films(films: list) -> list[ResponseFilm]:
    ...
    преобразования списка фильмов в ResponseFilm"""
    return [ResponseFilm(**film.dict()) for film in films]


def handle_no_films_error(films: list, query_params: dict):
    """Обрабатывает ошибку, если фильмы
    не найдены и добавляет информацию о запросе"""
    if not films:
        query_info = ", ".join(
...
async def film_list(
    ...
    handle_no_films_error(
        films, {"sort": sort, "page_size": page_size, "page": page}
    )
