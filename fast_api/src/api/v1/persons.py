...
async def person_list(
    ...
    persons = await person_service.get_person_list(page_size=page_size, page=page)
    if not persons:
        raise HTTPException(
            status_code=HTTPStatus.NOT_FOUND, detail="Ошибка при получении персон"
        )
    return persons
...
async def person_search(
    ...
    persons = await person_service.get_person_by_query(
        query=query, search_fields=search_fields, page_size=page_size, page=page
    )
    if persons is None:
        raise HTTPException(
...
async def person_details(
    ...
    if person is None:
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST, detail="Не найдена персона с таким ID"
        )
    return person
...
async def films_by_person(
    ...
    person_films = await film_service.get_person_films(person_id=person_id, nested_filters=nested_filters)
    if person_films is None:
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST, detail="Ошибка при получении фильмов"
        )
    return person_films
