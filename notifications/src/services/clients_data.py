...
class HttpxClientsDataService(AbstractDataClientsService):
    ...
    async def fetch_clients_data(self, uuids: list[UUID] | None = None, page_number: int | None = None) -> list[Client]:
        """
        ...
        try:
            ...
            logger.warning("Circuit Breaker: сервис аутентификации временно не доступен.")
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail="Сервис временно не доступен.",
            )
    ...
    async def get_clients_data(
        ...
        while True:
            ...
            logger.info(f"Получено {len(batch_clients_data)} пользователей на странице {page_number}")
