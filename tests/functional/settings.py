...
class TestSettings(BaseSettings):
    ...
    elastic_password: SecretStr | None = Field(
        None, validation_alias="ELASTIC_PASSWORD"
    )
    redis_host: str = Field("127.0.0.1", validation_alias="REDIS_HOST")
    redis_port: int = Field(6379, validation_alias="REDIS_PORT")
    redis_password: SecretStr | None = Field(
        None, validation_alias="REDIS_PASSWORD"
    )
    ...
    def get_elastic_url(self) -> str:
        auth_part = (
            f"{self.elastic_user}:{self.elastic_password.get_secret_value()}@"
