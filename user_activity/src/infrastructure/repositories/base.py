...
class AbstractRepository(ABC, Generic[T]):
    ...
    @abstractmethod
    async def get_by_user_id(self, user_id: UUID) -> list[T]: ...

    @abstractmethod
    async def update(self, T) -> T | None: ...
...
class BeanieBaseRepository(AbstractRepository[T], ABC):
    ...
    async def get_by_user_id(self, user_uid: UUID) -> list[T]:
        """
        ...
        documents = await self._model.find(self._model.user_uid == user_uid).to_list()
    ...
    async def update(self, T) -> T | None:
        ...
        document = await self._model.get(T.id)
        if document is None:
            return None
        document.update(T.model_dump())
        await document.save()
        document.id = str(document.id)
        return self._domain_model.model_validate(document)
